    ...
    <article className="group border-t border-[var(--border)] pt-4">
      <Link href={`/shop/${product.slug}`} className="block">
        <div className="relative aspect-[4/5] overflow-hidden bg-[var(--color-stone)]">
          <Image src={product.heroImage} alt={product.heroImageAlt ?? product.title} fill priority={priority} className="object-contain transition duration-500 group-hover:scale-[1.025]" sizes="(max-width: 639px) calc(100vw - 32px), (max-width: 1023px) calc((100vw - 80px) / 2), 420px" />
        </div>
        <div className="pt-4">
